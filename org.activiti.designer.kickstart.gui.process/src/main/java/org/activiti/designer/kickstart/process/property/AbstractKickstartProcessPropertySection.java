 ...
 */
public abstract class AbstractKickstartProcessPropertySection extends GFPropertySection
	...
	/**
	 * Shared focus-listener added to all controls created. Makes sure
	 * value-updates are performed and includes a fix for the focus-issue on
	 * Eclipse Juno.
	 */
		...
		focusListener = new FocusListener() {
			...
			public void focusGained(FocusEvent e) {
				if (e.widget instanceof Text) {
					// Fix for issue in Eclipse Juno where the text is filled
					// with value from the previous active
					// text-control in some cases. We set the value based on the
					// model on focus.
	...
	/**
	 * @param formComposite
	 *            the composite with a form-layout to add the controls to.
	 ...
	 *            model to get value from, is never null.
	...
	/**
	 * Method that is called when a control has a changed value and the model
	 * needs to be updated accordingly.
	 * 
	 * @param control
	 *            the control that has an updated value.
	 * @param businessObject
	 *            the model to update, is never null.
	 */
		...
		if (bo != null) {
			...
			if (hasChanged(oldValue, newValue)) {
				try {
					...
					// Perform the changes on the updatable BO instead of the
					// original
					...
					// Re-enable model-change listener after our change has been
					// applied
	...
	/**
	 * Execute the updates queued in the given updated using the current
	 * transactional edition domain.
	 */
	protected void executeModelUpdater() {
		// Make sure the update of the model is done in the transactional
		// editing domain
	...
	/**
	 * @return an {@link UpdateBusinessObjectCommand} that will be used to
	...
	protected KickstartProcessModelUpdater<?> createModelUpdater() {
		...
		if (featureProvider != null) {
			return featureProvider.getModelUpdaterFor(featureProvider.getBusinessObjectForPictogramElement(element),
					element);
		}
		return null;
	}
	...
	/**
	 * Clears the current updates, if any and creates a new fresh updater based
	...
	/**
	 * @param control
	 *            the control to set the value on, based on the given business
	 *            object.
	 * @param businessObject
	 */
	protected void populateControl(Control control, Object businessObject) {
		...
		if (control instanceof Text) {
			...
			// Set selected value on the button. Null model value is considered
			// to be 'false'
			if (valueFromModel == null) {
			...
			if (newIndex < 0) {
				// When no index is found, revert to default (if available) or
				// select first item
	...
	/**
	 * Override if custom resetting of components need to be done. Default
	 * implementation will clear existing values/selections.
	 * 
	 * @param control
	 *            the control to reset to an empty state, regardless of the
	 *            current selection.
	 */
	...
	/**
	 * Should be called when a control was added manually to the composite
	 * without using the create* methods.
	 */
	...
	/**
	 * @return a new {@link Text} control, added as last element of the given
	 *         composite, attached to the last control.
	 */
	protected Text createTextControl(boolean multiLine) {
		Text textControl = null;
		FormData data = null;
		if (multiLine == true) {
			textControl = getWidgetFactory().createText(formComposite, "",
	 ...
	 * @param defaultSelectionIndex
	 *            index of the default selection. If there is no default
	 *            selection, pass in a negative number.
